...
public class HttpClientUtil {
    ...
    public static JSONObject post(String url, Map<String, String> reqMap) {
        ...
        try {
            ...
            LogUtil.error(log, "httpclient exception:{}", e);
        }
        return null;
    }
    ...
    public static String post(String url, Map<String, String> reqMap, String encoding, SSLContext sslContext) {
        ...
        try {
            ...
            LogUtil.error(log, "url:{} post请求异常:{}", url, e.getMessage());
            e.printStackTrace();
        } finally {
            closeResource(httpClient, response);
        }
        ...
        try {
            if (response != null) {
                if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    LogUtil.info(log, "get请求404,未找到资源. url:" + url);
                } else if (response.getStatusLine().getStatusCode() == HttpStatus.SC_INTERNAL_SERVER_ERROR) {
                    LogUtil.info(log, "get请求500,服务器端异常. url:" + url);
                }
            }
        } catch (Exception e) {
            LogUtil.error(log, "url:" + url + ",处理响应，获取响应报文异常：" + e.getMessage());
            e.printStackTrace();
        } finally {
            if (br != null) {
                try {
    ...
    private static void closeResource(CloseableHttpClient httpClient, CloseableHttpResponse response) {
        if (response != null) {
            try {
                ...
                LogUtil.error(log, "释放response资源异常:" + e.getMessage());
                e.printStackTrace();
            }
        }
        ...
        if (httpClient != null) {
            try {
                ...
                LogUtil.error(log, "释放httpclient资源异常:" + e.getMessage());
                e.printStackTrace();
            }
        }
    }
    ...
    public static String doPost(String url, Map<String, String> headerMap, Map<String, String> paramMap, String bodyStr) {
        ...
        try {
            ...
            LogUtil.info(log,"doPost响应状态码：" + statusCode + "，结果：" + httpStr + "");
        } catch (IOException e) {
            LogUtil.error(log,"doPost执行异常：" + e.getMessage() + "", e);
        } finally {
            if (response != null) {
                try {
