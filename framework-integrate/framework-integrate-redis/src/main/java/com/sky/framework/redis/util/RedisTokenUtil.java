...
public class RedisTokenUtil {
    ...
    public static boolean validUserToken(String channel, String userId, String userToken) {
        ...
        String tokenValue = ObjectUtils.toString(redisService.getString(tokenKey));
        // 若Token为空，验证失败
        if (StringUtils.isBlank(tokenValue) || !userToken.equals(tokenValue)) {
            ...
            if (tokenExpireDay < 30) {
                redisService.expire(tokenKey, (RedisTokenUtil.TOKEN_EXPIRE * 60 * 60 * 24));
                log.info("重新设置用户token有效期，tokenKey={}, expireDay={}", tokenKey, RedisTokenUtil.TOKEN_EXPIRE);
            }
    ...
    public static String getUserToken(String channel, String userId) {
        ...
        String userToken = ObjectUtils.toString(redisService.getString(String.format(channel + ":" + TOKEN_KEY, userId)));
        if (StringUtils.isBlank(userToken)) {
            ...
            redisService.setString(String.format(channel + ":" + TOKEN_KEY, userId), userToken, (TOKEN_EXPIRE * 60 * 60 * 24));
            // 4.放入缓存token:userId
            redisService.setString(userToken, userId, (TOKEN_EXPIRE * 60 * 60 * 24));
        }
    ...
    public static void cancelUserToken(String userId) {
        ...
        // 2.查询key对应的value集合
        List<Object> valueList = redisService.multiGet(tokenKeys);
        // 3.删除该用户的所有token key
        for (Object tt : tokenKeys) {
            String vv = ObjectUtils.toString(redisService.getString(ObjectUtils.toString(tt)));
            if (vv != null) {
                redisService.deleteKey(vv);
            }
        }
        redisService.deleteKey(tokenKeys);
